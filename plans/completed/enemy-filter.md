...
**Created:** 2026-07-12
**Status:** ✅ COMPLETE — all steps shipped. Data (2.1/2.2) + engine (2.4) + UI (2.5) + retail cross-ref (2.3) + result-card polish (2.8) + **retail-divergence flag (2.7)** all committed & pushed to Pages. Property-dominance shipped as its own plan.
**Next Action:** None — plan complete. Move to `completed/`. v3 (`double-skillchain.md`) is now unblocked.
...
- [x] **2.7** *(follow-up, from 2.3)* **DONE.** Retail-divergence flag wired in `app.js`/`styles.css`. `retailDivergence(mob)` compares PDF `weak`/`strong` sets vs `retail.*` (order-independent `sameSet`); when they differ, the enemy bar shows a subtle amber **"retail differs"** pill that tap/clicks open a "weak — guide: X · retail: Y / source: …" note (click-to-reveal, not hover — works on touch). Source codes mapped to labels (allakhazam/bg/ffxiclopedia/both). **Validated:** node harness flags exactly the 8 documented divergences (118 clean + 4 no-retail + 8 = 130); Playwright at 390px — Khimaira shows pill → "guide: Earth · retail: Earth, Ice · BG Wiki + FFXIclopedia"; Colibri (non-divergent) shows no pill.
